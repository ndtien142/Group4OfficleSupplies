  ...
  return (
    <ScrollView>
      <View style={{ padding: 20 }}>
        <Text style={{ fontSize: 20, fontWeight: 'bold', marginBottom: 10 }}>Quản Lý Sản Phẩm</Text>
        <TextInput
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10 }}>
          ...
          <View style={{ flex: 1, marginLeft: 5 }}>
            <Button title="Sửa" onPress={editingIndex !== null ? () => editProduct(editingIndex, { type: productType, name: productName, price: productPrice, quantity: productQuantity, images: imageUris }) : () => {}} />
          </View>
        </View>
        <Button title="Chọn Hình Ảnh" onPress={selectImage} />
        <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
          {imageUris.map((uri, index) => (
            <Image key={index} source={{ uri }} style={{ width: 100, height: 100, marginRight: 10, marginBottom: 10 }} />
          ))}
        </View>
        {products.map((product, index) => (
          <View key={index} style={{ marginTop: 10, borderBottomWidth: 1, padding: 10 }}>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 10 }}>
              {product.images.map((uri, imgIndex) => (
                <Image key={imgIndex} source={{ uri }} style={{ width: 100, height: 100, marginRight: 10, marginBottom: 10 }} />
              ))}
            </View>
      ...
      <Button title="Thoát" onPress={clearFields} style={{ marginHorizontal: 20, marginBottom: 20 }} />
    </ScrollView>
  );
};
